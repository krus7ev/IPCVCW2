...
int main( int argc, const char** argv )
{
  ...
  houghCircle(grad_mag_uc, grad_dir, hough_uc, minRad, maxRad);
  cout<<"Generating Hough Line Transform"<<endl;
  houghLines(grad_mag_uc, grad_dir);
  
  int Ts = 150;
  filterBoxes (dartboards, frame, hough_uc, Ts, filename);
	return 0;
}
...
int filterBoxes (vector<Rect> &dartboards, Mat &frame, Mat &hough_uc, int Ts, string filename)
{
  ...
  int count = 0; 
    ...
    {
      for ( int x = dartboards[i].x; x < dartboards[i].x + dartboards[i].width ; y++)
      {
